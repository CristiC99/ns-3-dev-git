...
NS_LOG_COMPONENT_DEFINE ("ApWifiOffMac");

NS_OBJECT_ENSURE_REGISTERED (ApWifiOffMac);

TypeId
ApWifiOffMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ApWifiOffMac")
    .SetParent<OffloadWifiMac> ()
    .SetGroupName ("Wifi")
    .AddConstructor<ApWifiOffMac> ()
    .AddAttribute ("BeaconInterval",
                   "Delay between two beacons",
                   TimeValue (MicroSeconds (102400)),
                   MakeTimeAccessor (&ApWifiOffMac::GetBeaconInterval,
                                     &ApWifiOffMac::SetBeaconInterval),
                   MakeTimeChecker ())
    .AddAttribute ("BeaconJitter",
                   ...
                   MakePointerAccessor (&ApWifiOffMac::m_beaconJitter),
                   MakePointerChecker<UniformRandomVariable> ())
    .AddAttribute ("EnableBeaconJitter",
                   "If beacons are enabled, whether to jitter the initial send event.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&ApWifiOffMac::m_enableBeaconJitter),
                   MakeBooleanChecker ())
    .AddAttribute ("BeaconGeneration",
                   "Whether or not beacons are generated.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&ApWifiOffMac::SetBeaconGeneration,
                                        &ApWifiOffMac::GetBeaconGeneration),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableNonErpProtection", "Whether or not protection mechanism should be used when non-ERP STAs are present within the BSS."
                   ...
                   BooleanValue (true),
                   MakeBooleanAccessor (&ApWifiOffMac::m_enableNonErpProtection),
                   MakeBooleanChecker ())
  ;
  return tid;
}

ApWifiOffMac::ApWifiOffMac ()
{
...
void
ApWifiOffMac::SetAddress (Mac48Address address)
{
  ...
  OffloadWifiMac::SetAddress (address);
  OffloadWifiMac::SetBssid (address);
}

void
ApWifiOffMac::SetBeaconGeneration (bool enable)
{
  ...
  if (!enable)
    ...
    {
      m_beaconEvent = Simulator::ScheduleNow (&ApWifiOffMac::SendOneBeacon, this);
    }
  m_enableBeaconGeneration = enable;
}

bool
ApWifiOffMac::GetBeaconGeneration (void) const
{
  NS_LOG_FUNCTION (this);
...
Time
ApWifiOffMac::GetBeaconInterval (void) const
{
  NS_LOG_FUNCTION (this);
...
void
ApWifiOffMac::SetWifiRemoteStationManager (Ptr<WifiRemoteStationManager> stationManager)
{
  NS_LOG_FUNCTION (this << stationManager);
  m_beaconDca->SetWifiRemoteStationManager (stationManager);
  OffloadWifiMac::SetWifiRemoteStationManager (stationManager);
}

void
ApWifiOffMac::SetLinkUpCallback (Callback<void> linkUp)
{
  NS_LOG_FUNCTION (this << &linkUp);
  OffloadWifiMac::SetLinkUpCallback (linkUp);
...
void
ApWifiOffMac::SetBeaconInterval (Time interval)
{
...
void
ApWifiOffMac::StartBeaconing (void)
{
  NS_LOG_FUNCTION (this);
  SendOneBeacon ();
}
...
int64_t
ApWifiOffMac::AssignStreams (int64_t stream)
{
...
bool
ApWifiOffMac::GetShortSlotTimeEnabled (void) const
{
...
bool
ApWifiOffMac::GetShortPreambleEnabled (void) const
{
...
bool
ApWifiOffMac::IsNonGfHtStasPresent (void) const
{
  for (std::list<Mac48Address>::const_iterator i = m_staList.begin (); i != m_staList.end (); i++)
  {
...
void
ApWifiOffMac::ForwardDown (Ptr<const Packet> packet, Mac48Address from,
                        Mac48Address to)
{
...
void
ApWifiOffMac::Enqueue (Ptr<const Packet> packet, Mac48Address to, Mac48Address from)
{
...
void
ApWifiOffMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
...
bool
ApWifiOffMac::SupportsSendFrom (void) const
{
  NS_LOG_FUNCTION (this);
  return true;
}
...
SupportedRates
ApWifiOffMac::GetSupportedRates (void) const
{
  NS_LOG_FUNCTION (this);
...
DsssParameterSet
ApWifiOffMac::GetDsssParameterSet (void) const
{
...
ApWifiOffMac::GetCapabilities (void) const
{
...
ErpInformation
ApWifiOffMac::GetErpInformation (void) const
{
...
EdcaParameterSet
ApWifiOffMac::GetEdcaParameterSet (void) const
{
...
HtOperations
ApWifiOffMac::GetHtOperations (void) const
{
...
void
ApWifiOffMac::SendProbeResp (Mac48Address to)
{
...
void
ApWifiOffMac::SendAssocResp (Mac48Address to, bool success)
{
...
void
ApWifiOffMac::SendOneBeacon (void)
{
  NS_LOG_FUNCTION (this);
  WifiMacHeader hdr;
  hdr.SetBeacon ();
  ...
  m_beaconEvent = Simulator::Schedule (m_beaconInterval, &ApWifiOffMac::SendOneBeacon, this);
...
void
ApWifiOffMac::TxOk (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this);
  OffloadWifiMac::TxOk (hdr);
...
void
ApWifiOffMac::TxFailed (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this);
  OffloadWifiMac::TxFailed (hdr);
...
void
ApWifiOffMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  ...
  OffloadWifiMac::Receive (packet, hdr);
}

void
ApWifiOffMac::DeaggregateAmsduAndForward (Ptr<Packet> aggregatedPacket,
...
void
ApWifiOffMac::DoInitialize (void)
{
  NS_LOG_FUNCTION (this);
      ...
      if (m_enableBeaconJitter)
        {
          ...
          m_beaconEvent = Simulator::Schedule (MicroSeconds (jitter), &ApWifiOffMac::SendOneBeacon, this);
        }
      else
        {
...
bool
ApWifiOffMac::GetUseNonErpProtection (void) const
{
